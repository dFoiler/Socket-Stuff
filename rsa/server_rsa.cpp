...
{
	// Setup and send RSA cryptosystem
	mzt_t N,e,d; mzt_inits(N,e,d,NULL);
	setup_rsa(MAXSATASIZE*8, N, e, d); // Input number of bits
	send(cleint_desc, mpz_get_str(N), MAXDATASIZE, 0);
	send(client_desc, mpz_get_str(e), MAXDATASIZE, 0);
